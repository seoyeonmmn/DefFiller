...
def prepare_batch_sem(meta, batch=1):
    ...
    sem = sem.resize( (256, 256), Image.NEAREST ) 
...
def run(meta, config, starting_noise=None):
    ...
    for image_id, sample in zip(image_ids, samples_fake):
        img_name = os.path.basename(meta["sem"]).replace('.png', f'_fake.png')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mask_dir", type=str,  default='./DATA/SD-saliency-900/Ground_truth')
    parser.add_argument("--prompt", type=str,  default='scratches')
    parser.add_argument("--save_folder_name", type=str,  default='scratches')
    ...
    for file in files:
        meta_list.append( dict(
            ckpt ="./OUTPUT/sd/tag02/sem_checkpoint_latest.pth",
            prompt = args.prompt, 
            sem = file, 
